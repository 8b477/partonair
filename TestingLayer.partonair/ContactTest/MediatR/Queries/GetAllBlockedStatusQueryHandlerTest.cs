...
{
    public class GetAllBlockedStatusQueryHandlerTest : BaseContactApplicationMediatRTestFixture<GetAllBlockedStatusQueryHandler>
    {
        ...
        public GetAllBlockedStatusQueryHandlerTest()
        {
            _contact1 = new(
                Guid.NewGuid(),
                Guid.NewGuid(),
                ...
                DateTime.Now,
                DateTime.Now,
                StatusContact.Blocked.ToString()
                );
        ...
        [Fact]
        public async Task GetAllBlockedStatusQueryHandler_ShouldReturn_Success_ICollection_ContactViewDto()
        {
            // Arrange
            _mockContactService.Setup(s => s.GetAllBlockedStatusAsyncService(It.IsAny<string>()))
                               .ReturnsAsync(_contactViews);

            // Act
            var result = await _handler.Handle(new GetAllBlockedStatusQuery(It.IsAny<string>()), CancellationToken.None);
            ...
            _mockContactService.Verify(v => v.GetAllBlockedStatusAsyncService(It.IsAny<string>()), Times.Once);
        }

        [Fact]
        public async Task GetAllBlockedStatusQueryHandler_ShouldThrow_ApplicationLayerException_ConstraintViolationErrorException()
        {
            // Arrange
            _mockContactService.Setup(s => s.GetAllBlockedStatusAsyncService(It.IsAny<string>()))
            ...
            var exception = await Assert.ThrowsAsync<ApplicationLayerException>(()
                => _handler.Handle(new GetAllBlockedStatusQuery(It.IsAny<string>()), CancellationToken.None));
        ...
        [Fact]
        public async Task GetAllBlockedStatusQueryHandler_ShouldThrow_InfrastructureLayerException_CancelationDatabaseException()
        {
            // Arrange
            _mockContactService.Setup(s => s.GetAllBlockedStatusAsyncService(It.IsAny<string>()))
