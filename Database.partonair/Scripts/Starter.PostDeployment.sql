...
BEGIN
    INSERT INTO [Profiles](Id, ProfileDescription)
    VALUES
    (@ProfileEmployee1Guid ,N'Designer depuis 3 ans, j''aime créer des templates originaux et relever de nouveaux défis. N''hésitez pas à me contacter pour vos projets créatifs.'),
    (@ProfileEmployee2Guid ,N'Développeur full-stack avec 5 ans d''expérience. Passionné par les technologies web modernes et l''architecture logicielle.'),
    (@ProfileEmployee3Guid ,N'Jeune entreprise qui recherche tout forme de profils n''hésiter donc pas à nous contacter par message priver ou par mail.'),
    (@ProfileEmployee4Guid ,N'Spécialisé dans la transformation digitale des entreprises. Fort d''une expérience dans divers secteurs d''activité.'),
    (@ProfileCompany5Guid  ,N'Spécialiste en CI/CD, containerisation et orchestration. Passionné par l''amélioration continue des processus de déploiement.'),
    (@ProfileCompany6Guid  ,N'Notre entreprise PartOnAir, spécialisée dans la mise en relation, cherche activement des profils tech. Nous connectons les talents IT aux meilleures opportunités du marché, en mettant l''accent sur l''innovation et la croissance professionnelle.');
END
...
BEGIN
    ...
    VALUES 
        ...
        (@User2Guid  ,'jane'    ,'jane@mail.be'    ,'Test1234*' ,1 ,'Employee' ,GETDATE() ,GETDATE() ,@ProfileEmployee1Guid ),
        (@User3Guid  ,'bob'     ,'bob@mail.be'     ,'Test1234*' ,1 ,'Company'  ,GETDATE() ,GETDATE() ,@ProfileCompany5Guid  ),
        (@User4Guid  ,'alice'   ,'alice@mail.be'   ,'Test1234*' ,0 ,'Visitor'  ,GETDATE() ,GETDATE() ,NULL                  ),
        (@User5Guid  ,'charlie' ,'charlie@mail.be' ,'Test1234*' ,1 ,'Employee' ,GETDATE() ,GETDATE() ,@ProfileEmployee2Guid ),
        (@User6Guid  ,'diana'   ,'diana@mail.be'   ,'Test1234*' ,1 ,'Employee' ,GETDATE() ,GETDATE() ,@ProfileEmployee3Guid ),
        (@User7Guid  ,'evan'    ,'evan@mail.be'    ,'Test1234*' ,0 ,'Visitor'  ,GETDATE() ,GETDATE() ,NULL                  ),
        (@User8Guid  ,'fiona'   ,'fiona@mail.be'   ,'Test1234*' ,1 ,'Company'  ,GETDATE() ,GETDATE() ,@ProfileCompany6Guid  ),
        (@User9Guid  ,'george'  ,'george@mail.be'  ,'Test1234*' ,1 ,'Employee' ,GETDATE() ,GETDATE() ,@ProfileEmployee4Guid ),
